...
knitr::opts_chunk$set(
  ...
  fig.width = 7, fig.height = 4, dev = "ragg_png", dpi = 150,
  fig.path = "workflow-vignette-",
...
```{r}
rozv_path_fn <- funtcion(dataset_id, year, month, url) {
  sp_get_dataset("rozv", 2019, 12)
  }

rozv_url <- sp_get_dataset_url("rozv", 2019, 12) # for tracking URL if needed
...
This step-by-step workflow is suitable for being turned into an explicit pipeline e.g. using the [{targets}](https://wlandau.github.io/targets/) framework.
...
> Note the {targets} package is in active development; this script works on the 0.0.9004 version ().
...
# provide a list as output of the entire script
list(targets_codelists, targets_data)
```
...
The targets workflow will also improve performance of codelist reading, since codelists will be stored in {targets}' fast datastore. In contrast, `sp_get_codelist()` must reread the XML time every time it is called, so even if the XML file has been downloaded previously, there is some overhead coming from the XML parsing.
