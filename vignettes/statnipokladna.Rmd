...
vignette: >
  %\VignetteIndexEntry{statnipokladna}
...
> Note for Czech speakers: a previous note covering similar information, sometimes in more depth, is available in Czech [the Czech guide to the data](how-the-data-works-cz.html).
...
The `sp_add_codelist()` function tries to take care of this for you, but may not success if there are oddities in the data, e.g. one code with two items valid for the same date. In that case you get an informative error message; you should then resolve the duplicates manually using data returned by `sp_get_codelist()` and supply the edited data to `sp_add_codelist()` as an object.
...
Budget data has a special form: the available data files provide all sorts of breakdowns in a single file, in long format, crossed between each other. This means that any individual value will make little sense - it will be e.g. money from a particular sector, from a particlar source, either capital or current. Each of these categorisations - of which there are a few more - have multiple levels of detail.
